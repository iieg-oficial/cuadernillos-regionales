...
def get_valor_produccion_agricola_anual(session: Session, cve_mun: int):
    stmt = text("""
        SELECT
            anio,
...
def get_valor_produccion_agricola_municipal(session: Session, cve_mun: int, anio: int):
    stmt = text("""
        SELECT ROUND(SUM(valor_produccion)::numeric / 1000000, 2) AS valor_mdp
        FROM public.stg_agricola
        WHERE entidad_id = 14
...
def get_ultimo_anio_agricola(session: Session):
    stmt = text("""
